...
use windows::{
    ...
    core::{Error, HRESULT, PCSTR, Result, s},
};
...
unsafe fn load_func(function_string: CString) -> *const c_void {
    ...
    let module = OPENGL3_LIB.get_or_init(|| {
        LoadLibraryA(s!("opengl32.dll\0"))
            .expect("LoadLibraryA")
            .into()
    });
...
impl RenderContext for OpenGl3RenderEngine {
    fn load_texture(&mut self, data: &[u8], width: u32, height: u32) -> Result<TextureId> {
        unsafe {
            self.texture_heap
                .create_texture(&self.gl, data, width, height)
        }
    }
    ...
    fn replace_texture(
        ...
        unsafe {
            self.texture_heap
                .update_texture(&self.gl, texture_id, data, width, height)
        }
    }
}
