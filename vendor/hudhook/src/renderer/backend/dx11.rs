use std::{ffi::c_void, mem, mem::offset_of, ptr, slice};
...
use windows::{
    ...
    core::{Error, HRESULT, Result, s},
};

use crate::{RenderContext, renderer::RenderEngine, util};
    ...
    fn render(
        ...
        unsafe {
            ...
            let render_target: ID3D11RenderTargetView = util::try_out_ptr(|v| {
                self.device
                    .CreateRenderTargetView(&render_target, None, Some(v))
            })?;
...
impl D3D11RenderEngine {
    unsafe fn render_draw_data(&mut self, draw_data: &DrawData) -> Result<()> {
        ...
        draw_data
            ...
            .map(|draw_list| {
                (
                    draw_list.vtx_buffer().iter().copied(),
        ...
        self.vertex_buffer
            .upload(&self.device, &self.device_context)?;
        self.index_buffer
            .upload(&self.device, &self.device_context)?;
            ...
            for cmd in cl.commands() {
                match cmd {
                    ...
                    DrawCmd::RawCallback {
                        callback,
                        raw_cmd,
                    } => unsafe { callback(cl.raw(), raw_cmd) },
                }
            }
    ...
    unsafe fn setup_render_state(&self, draw_data: &DrawData) {
        ...
        self.device_context
            .IASetInputLayout(&self.shader_program.input_layout);
        ...
        self.device_context
            .VSSetShader(&self.shader_program.vertex_shader, Some(&[]));
        self.device_context
            .VSSetConstantBuffers(0, Some(&[Some(self.projection_buffer.resource.clone())]));
        self.device_context
            .PSSetShader(&self.shader_program.pixel_shader, Some(&[]));
        self.device_context
        ...
        self.device_context
            .OMSetDepthStencilState(&self.shader_program.depth_stencil_state, 0);
        self.device_context
            .RSSetState(&self.shader_program.rasterizer_state);
    }
}
    ...
    unsafe fn restore(self, device_context: &ID3D11DeviceContext) {
        ...
        device_context.PSSetShader(
            ...
            if self.ps_instances_count > 0 {
                Some(slice::from_raw_parts(
                    self.ps_instances,
                    self.ps_instances_count as usize,
                ))
            } else {
                None
            },
        ...
        device_context.VSSetShader(
            ...
            if self.vs_instances_count > 0 {
                Some(slice::from_raw_parts(
                    self.vs_instances,
                    self.vs_instances_count as usize,
                ))
            } else {
                None
            },
